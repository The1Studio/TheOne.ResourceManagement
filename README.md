...
**Sync (explicit key - object type, typically string):**
- `T Load<T>(object key) where T : Object`
- `IEnumerable<T> LoadAll<T>(object key) where T : Object`
- `bool TryLoad<T>(object key, out T asset) where T : Object`
- `T LoadComponent<T>(object key)` / `IEnumerable<T> LoadAllComponents<T>(object key)`
- `bool TryLoadComponent<T>(object key, out T component)`
- `void Unload(object key)` / `void UnloadAll(object key)`

**Sync (implicit key - uses typeof(T).GetKey()):**
- `T Load<T>()` / `IEnumerable<T> LoadAll<T>()` / `bool TryLoad<T>(out T asset)`
- `T LoadComponent<T>()` / `IEnumerable<T> LoadAllComponents<T>()` / `bool TryLoadComponent<T>(out T component)`
- `void Unload<T>()`

**Async (UniTask, explicit key):**
- `UniTask<T> LoadAsync<T>(object key, IProgress<float>? = null, CancellationToken = default)`
- `UniTask<IEnumerable<T>> LoadAllAsync<T>(object key, ...)`
- `UniTask<(bool IsSucceeded, T Asset)> TryLoadAsync<T>(object key, ...)`
- `UniTask<T> LoadComponentAsync<T>(object key, ...)` / `UniTask<IEnumerable<T>> LoadAllComponentsAsync<T>(...)`
- `UniTask<(bool IsSucceeded, T Component)> TryLoadComponentAsync<T>(object key, ...)`

**Async (UniTask, implicit key):** all above without key parameter

**Async (Coroutine):** all above with `Action<T> callback` instead of return type

## IRemoteAssetsDownloader (implemented by AddressableAssetsManager)
- `DownloadAsync(object key, progress?, cancellationToken?)` - download remote Addressable assets to device
- `DownloadAllAsync(progress?, cancellationToken?)` - download all remote content
- `DownloadAsync<T>(progress?, cancellationToken?)` - implicit key variant
- Coroutine variants with `Action? callback`

## IScenesManager
- `void Load(string name, LoadSceneMode mode = Single)`
- `LoadAsync(string name, LoadSceneMode mode = Single, progress?, cancellationToken?)` - UniTask
- `UnloadAsync(string name, progress?, cancellationToken?)` - UniTask
- Coroutine variants with `Action? callback`

## IExternalAssetsManager
- `DownloadTextAsync(string url, bool cache = true, progress?, cancellationToken?)` - returns string
- `DownloadBufferAsync(string url, bool cache = true, ...)` - returns byte[]
- `DownloadTextureAsync(string url, bool cache = true, ...)` - returns Texture2D
- `DownloadSpriteAsync(string url, bool cache = true, ...)` - returns Sprite
- `DownloadAudioClipAsync(string url, AudioType audioType, bool cache = true, ...)` - returns AudioClip
- `DownloadFileAsync(string url, string savePath, bool cache = true, ...)`
- All methods also accept `Uri` overloads
- `void DeleteCache(string key)`
- Coroutine variants with `Action<T> callback`

## Implementations
- `AddressableAssetsManager` (sealed) - `[Preserve] (ILoggerManager, string? scope = null)` - also implements IRemoteAssetsDownloader
- `ResourceAssetsManager` (sealed) - `[Preserve] (ILoggerManager, string? scope = null)` - string keys only
- `AddressableScenesManager` (sealed) - `[Preserve] (ILoggerManager)`
- `ResourceScenesManager` (sealed) - `[Preserve] (ILoggerManager)`
- `ExternalAssetsManager` (sealed) - `[Preserve] (ILoggerManager)`
...
Automatically selects Addressable (THEONE_ADDRESSABLES) or Resource implementation.
